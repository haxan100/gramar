      ...
      init : function(ed, url) 
      {
         ...
         editor.addCommand('mceWritingImprovementTool', function(languageCode)
         {
            ...
            plugin.sendRequest('', textContent, languageCode, function(data, request, jqXHR)
            {
               ...
               $("#grammdata").val(jqXHR.responseText);
               ...
               if (json && json.matches) {
                   if (json['hiddenMatches']) {
                       ...
                       if (json.matches.length>0) {
                            $('#feedbackMessage').html(json.matches.length+' errors found. &nbsp; &nbsp; <span  class="solAll btn btn-info">Resolve All</span>');
                    ...
                    onclick : function() {
                        ...
                        jQuery.getJSON("/online-check/tiny_mce/plugins/atd-tinymce/server/rule-proxy.php?lang="
                            ...
                            function(data) {
                                ...
                                if (exampleCount === 0) {
                                    ruleHtml += "<p id='gdata'>" + noRuleExamples + "</p>";
                                    t._trackEvent('ShowExamples', 'NoExamples', ruleId);
                                }
                                ruleHtml += "<p id='gdata'><a target='_lt_rule_details' href='" + ruleUrl + "'>" + ruleImplementation + "</a></p>";
                                var $dialog = $("#dialog");
