window.bridges["convert-string-to-image"] = function(){
    ...
    var bridge = function(string) {
        ...
        if (result.width != 0 && result.height != 0) {
            ...
            var fit = best_image_fit(result.width, result.height, canvas_preview.width, canvas_preview.height);
            ...
            tool.output.showWarningBadge("No image data", "Image {0} is zero.".format(which), -1);
        }
    ...
    function wrapText(opts) {
        ...
        for (var i = 0; i < parts.length; i++) {
            ...
            ctx.fillText(part, imagePadding + parseInt(x), imagePadding + parseInt(y));
    ...
    function parseOptions (tool) {
        ...
        if (lineHeight < 0) {
            error("Can't convert to image.", "Line height must be a positive number.");
            return false;
        }
        ...
        return {
            ...
            color: isColorValid(options["text-color"]) ? options["text-color"] : "black",
            background: isColorValid(options["background-color"]) ? options["background-color"] : "transparent",
            align: {
    ...
    return {
        ...
        config: {
            type: "image",
            output: {
                download: function(){ return this.options.get()["format"] },
                noClipboard: true
            }
        }
    }
